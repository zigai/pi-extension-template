...
test:
    ...
    grep -q 'from "vitest"' .tmp/pi-template-smoke/test/index.test.ts
    grep -q '"test": "vitest run"' .tmp/pi-template-smoke/package.json
    grep -q '"coverage": "vitest run --coverage"' .tmp/pi-template-smoke/package.json
    grep -q '"@vitest/coverage-v8": "^4.1.9"' .tmp/pi-template-smoke/package.json
    grep -q '"vitest": "^4.1.9"' .tmp/pi-template-smoke/package.json
    ! grep -q '"tsx"' .tmp/pi-template-smoke/package.json
